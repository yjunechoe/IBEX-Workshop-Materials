...
meta() %>% 
  meta_social(
    ...
    url = "https://ibex-workshop-slides.netlify.app/",
    image = "https://raw.githubusercontent.com/yjunechoe/IBEX-Workshop-Materials/master/Card.png",
...
```{css, echo = FALSE}
.title-slide {
  background-image: url(https://raw.githubusercontent.com/yjunechoe/IBEX-Workshop-Materials/master/Card.png);
...
<h1 style="text-align:left; margin:15px 0px 0px 0px">Guest</h1>

<img src="https://yjunechoe.github.io/static/img/profile.png" width="150px">



<h2 style="margin:15px 0px 3px 0px"> June Choe</h2>

<p style="margin:0px"><a style="color:white" href="https://yjunechoe.github.io">yjunechoe.github.io</a></p>

<h3 style="margin:25px 0px 10px 0px">Ph.D. Student in Linguistics</h3>

<p style="margin-top:0px; padding-bottom:30px; border-bottom: 1px solid white">University of Pennsylvania (Philadelphia, PA, USA)</p>

<strong style="color:white">Research</strong>: psycholinguistics, computational linguistics

<p style="font-size:16px; margin-top:2em; margin-bottom: 0px">Will be answering questions in the chat!</p>
...
We'll use **our own** - <strong><a href="ibex_files/full_exp.js">template file</a></strong> - to demonstrate how the script works.

<embed style='border: 1px solid black' type="text/html" src="ibex_files/full_exp.js"  width="800" height="300">

The above script creates [this acceptability rating experiment](https://spellout.net/ibexexps/skku_ibex/workshop_acceptabilityJudgment/experiment.html).
...
This means that the first element of the big bracket can just be the _Trial Name_, without assigning it a _Set #_:
...
**Note**: You can show multiple lines of text using .bl[html] tags like "p" and "div" - more on that later
...
The "p" is called an .bl[HTML tag] and there are [many others](https://www.w3schools.com/TAGs/) (most common: "div", "strong", "em"), but usually these don't get very complicated.
...
Some Controllers return more than 1 line of results, making thing complicated.

--

For example, each **AcceptabilityJudgment** trial adds two lines to _results_:
...
In cases like these, using a .bl[processing script (R/Python)] is recommended
...
1. Download the _results_ file from IBEX

2. Open it with the `read_ibex()` function from this [read_ibex.R](https://github.com/yjunechoe/IBEX-Workshop-Materials/blob/master/R%20codes/read_ibex.R) script
...
SPR_results <- read_ibex("ibex_files/SPR_results.txt")
# Sample a couple lines and exclude some columns
SPR_results[sample(nrow(SPR_results), 3), -c(1, 2, 4, 5, 11)]
```
...
```{r, echo = FALSE}
source("R codes/read_ibex.R")
Acceptability_results <- read_ibex("ibex_files/Acceptability_results.txt")
# Sample a couple lines and exclude some columns
Acceptability_results[sample(nrow(Acceptability_results), 3), -c(1, 2, 4, 5, 9, 11, 12)]
```
...
2. Create a new experiment on your ibex account called "WorkshopExercise" and replace the contents of <em>example_data.js</em> in the <strong>data_includes</strong> section with <em>incomplete.js</em>.

3. Click .blue[edit] and follow the directions in the script to fill in the blanks.

4. Complete as many of tasks as possible (.bl[8 total]).

If you finish, save the edits and click on your experiment link at the top of the page. Check to see that your experiment looks similar to this [complete version](https://spellout.net/ibexexps/skku_ibex/workshop_exercise/experiment.html).

<p style='padding-top:25px'><strong>Make sure to use the IBEX documentation!</strong></p>
...
- The official [IBEX documentation](https://github.com/addrummond/ibex/blob/master/docs/manual.md).

- The official [IBEX google group](https://groups.google.com/g/ibexexperiments/c/-PTeVD39h9o) to ask questions. 

--

<p style = 'padding-top:25px'><strong>UPDATE (01/01/2021)</strong></p>

There is a **new version of IBEX** at a different link: [https://ibex.spellout.net](https://ibex.spellout.net)

```{r, echo=F, out.width = '650px', out.extra="style='border:1px solid black; margin:auto; display:block;'"}
knitr::include_graphics("img/ibex_new.png")
```
...
**New feature**: results can be downloaded as a spreadsheet ([full list of changes](https://gist.github.com/addrummond/b345d3d4f23436838e52afbe880ebbd9)).

.pull-left[
```{r, echo=F, out.width = '400px', out.extra="style='border:1px solid black; margin:auto; display:block;'"}
knitr::include_graphics("img/new_feature_1.png")
```
]

.pull-right[
```{r, echo=F, out.width = '400px', out.extra="style='border:1px solid black; margin:auto; display:block;'"}
knitr::include_graphics("img/new_feature_2.png")
```
]
...
<p style = 'padding-top:25px'><strong style='color:black'>Other platforms</strong></p>

- PCIbex (Penn Controller for IBEX): [https://www.pcibex.net](https://www.pcibex.net)

- PsychoPy: [https://www.psychopy.org](https://www.psychopy.org)

- _magpie (minimal architecture for the generation of portable interactive experiments): [https://magpie-ea.github.io/magpie-site](https://magpie-ea.github.io/magpie-site)
...
<p style = 'padding-top:25px'><strong style='color:black'>Platforms to recruit participants for your IBEX experiment</strong></p>

- MTurk (Amazon Mechanical Turk): [https://www.mturk.com](https://www.mturk.com)

- Prolific [https://www.prolific.co](https://www.prolific.co)

---

# Acknowledgments

- Many thanks to June Choe for assistance with the slides (esp. interactive components)

- Thanks to Brian Dillon (University of Massachusetts Amherst) for permission to use materials from the LSA Minicourse "Doing Experiments for Linguists" (Brian Dillon & Rodica Ivan)
