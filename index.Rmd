...
The first element of the big bracket can just be the _Trial Name_, without assigning it a _Item Set #_ (only relevant for counterbalancing, more later):
...
To alternate between participants, we need to define a special .bl[counter] "trial":
...
For example, using the `defaults` variable, we can specify parameters for the **method design**.
...
- <p>We use a templatic syntax for creating stimuli</p>

- We store all the materials for our experiment inside `items`

- We specify the order of presentation inside `shuffleSequence`

- We can set various options for the experiment, such as defaults for the _task design_.
...
The data collected in the experiment can be found on the experiment page, in the **Results** tab.

```{r, echo=F, out.width = '600px', out.extra="style='border:1px solid black; margin:auto; display:block;'"}
knitr::include_graphics("img/results_section.png")
```

That gives you a preview of the results and gives you options to download or delete the collected data.
...
The lines that start with a pound symbol "#" are **comments** that include _metadata_.
...
IBEX is always good to know due to the sheer amount of linguistics experiments already written in IBEX. PCIBex is not difficult to pick up after IBEX, should you choose to learn it.
