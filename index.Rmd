...
For more complex trials, combine multiple controllers:

.panelset[
.panel[.panel-name[DashedSentence+Question]

Add a .bl[Question] controller to ask a comprehension question:

```{r, eval = F}
[["gp.trans",1],
"DashedSentence", {s: "While Anna trained the kitten paid attention."},
"Question", {q: "Was the kitten trained?", as: ["Yes", "No"]}] #<<
```

```{r, echo=F, out.width = '200px', fig.align='center'}
...
.panel[.panel-name[DashedSentence+Form]

Add a .bl[Form] controller with a "textarea" element to collect a free response:

```{r, eval = F}
[["gp.trans",1],
"DashedSentence", {s: "While Anna trained the kitten paid attention."},
"Form", {html: ["div", #<<
   ["p", "In your own words, paraphrase the sentence you just read."], #<<
   ["textarea"] #<<
]}] #<<
```

```{r, echo=F, out.width = '400px', fig.align='center'}
knitr::include_graphics("img/spr_form.png")
```

]

.panel[.panel-name[Message+Question]

.bl[AcceptabilityJudgment] is just a combination of .bl[Message] and .bl[Question]!

```{r, eval = F}
[["gp.trans",1],
"Message", {html: "While Anna trained the kitten paid attention."},
"Question", { 
  as: ["1", "2", "3", "4", "5", "6", "7"], 
  presentAsScale: true, 
  instructions: "Use number keys or click boxes to answer.", 
...
Because of this, .bl[AcceptabilityJudgment] trial adds two lines to the _results_ file - one line from .bl[Message] and one line from .bl[Question] (more on this later)
...
2. Create a new experiment on your ibex account called "WorkshopExercise" and replace the contents of <em>example_data.js</em> in the **data_includes** section with _incomplete.js_.
